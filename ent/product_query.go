...
func (pq *ProductQuery) QueryStyle() *StyleQuery {
	query := &StyleQuery{config: pq.config}
...
// QuerySize chains the current query on the "size" edge.
func (pq *ProductQuery) QuerySize() *SizeQuery {
	query := &SizeQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withStyle:  pq.withStyle.Clone(),
		withSize:   pq.withSize.Clone(),
...
func (pq *ProductQuery) WithStyle(opts ...func(*StyleQuery)) *ProductQuery {
	query := &StyleQuery{config: pq.config}
...
// WithSize tells the query-builder to eager-load the nodes that are connected to
// the "size" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithSize(opts ...func(*SizeQuery)) *ProductQuery {
	query := &SizeQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if pq.withStyle != nil || pq.withSize != nil || pq.withGender != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withStyle; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Product)
		for i := range nodes {
			if nodes[i].style_id == nil {
				continue
			}
			fk := *nodes[i].style_id
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "style_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Style = n
			}
		}
	}

	if query := pq.withSize; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Product)
		for i := range nodes {
			if nodes[i].size_id == nil {
				continue
			}
			fk := *nodes[i].size_id
			if _, ok := nodeids[fk]; !ok {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "size_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Size = n
			}
		}
	}
	...
	if query := pq.withGender; query != nil {
		...
		nodeids := make(map[int][]*Product)
		for i := range nodes {
			if nodes[i].gender_id == nil {
				continue
			}
			fk := *nodes[i].gender_id
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
